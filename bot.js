...
setInterval(() => {
  http.get(`http://protacshon.glitch.me/`);
}, 280000);
...
let anti = JSON.parse(fs.readFileSync("./antigreff.json", "UTF8"));//dolimi
...
client.on("message", message => {
    ...
    if(!config[message.guild.id]) config[message.guild.id] = {
        banLimit: 3,
        chaDelLimit: 3,
        roleDelLimit: 3,//dolimi
        kickLimits: 3,
        roleCrLimits: 3,
        time: 30
    }
if(message.content.startsWith(prefix + "limit")) {//dolimi

 
    if(!message.member.hasPermission('MANAGE_GUILD')) return;
    if(message.content.startsWith(prefix + "limitbans")) {
        if(!num) return message.channel.send("**→ | Supply a number !");
        if(isNaN(num)) return message.channel.send("**→ | Supply a number !**");//dolimi
        config[message.guild.id].banLimit = num;
        message.channel.send(`**→ | Changed bans limit to : ${config[message.guild.id].banLimit}.**`)//dolimi
    }
    ...
    if(message.content.startsWith(prefix + "limitroleCreate")) {
        if(!num) return message.channel.send("**→ | Supply a number !**");//dolimi
        if(isNaN(num)) return message.channel.send("**→ | Supply a number !**");
    ...
    if(message.content.startsWith(prefix + "limittime")) {
        if(!num) return message.channel.send("**→ | Supply a number !**");
        if(isNaN(num)) return message.channel.send("**→ | Supply a number !**");
        config[message.guild.id].time = num;//dolimi//dolimi
...
client.on("channelDelete", async channel => {
    ...
    const entry = entry1.executor//dolimi
    if (!config[channel.guild.id]) config[channel.guild.id] = {
        banLimit: 3,
        chaDelLimit: 3,
        roleDelLimit: 3,
        kickLimits: 3,
        roleCrLimits: 3//dolimi
    }
    if (!anti[channel.guild.id + entry.id]) {//dolimi
        ...
        anti[channel.guild.id + entry.id].actions = Math.floor(anti[channel.guild.id + entry.id].actions + 1)//dolimi
        console.log("TETS");
        setTimeout(() => {
            anti[channel.guild.id + entry.id].actions = "0"
        }, config[channel.guild.id].time * 1000)//dolimi
        if (anti[channel.guild.id + entry.id].actions >= config[channel.guild.id].chaDelLimit) {
            ...
            anti[channel.guild.id + entry.id].actions = "0"
            fs.writeFile("./config.json", JSON.stringify(config, null, 2), function (e) {//dolimi
                if (e) throw e;
            });//dolimi
            fs.writeFile("./antigreff.json", JSON.stringify(anti, null, 2), function (e) {
                if (e) throw e;
...
●▬▬▬▬▬▬▬▬▬๑۩۩๑▬▬▬▬▬▬▬▬▬▬●
-@@----|**DEVELOBER BY MOHAMMED ALDOLIMI**|--@@--
●▬▬▬▬▬▬▬▬▬๑۩۩๑▬▬▬▬▬▬▬▬▬▬●
--@**اظغط على الاوامر المخفية لروئيتها**@--
●▬▬▬▬▬▬▬▬▬๑۩۩๑▬▬▬▬▬▬▬▬▬▬●
1--- ||${prefix}limitbans|| ==**لتحديد الحد الاقصى للباند**

2--- ||${prefix}limitkicks|| ==**لتحديد الحد الاقصى للكيك **

3--- ||${prefix}limitroleDelete|| == **لتحديد الحد الاقصى من حذف الرولات**

4--- ||${prefix}limitchannelDelete|| == **الحد الاقصى لحذ الرومات**

5--- ||${prefix}antibots on|| == **لمنع دخول لبوتات **

6--- ||${prefix}antibots off|| == **لايقاف منع دخمل البوتات** 

7--- ||${prefix}limittime 1000|| == **تكتبها بعد متخلص **
●▬▬▬▬▬▬▬▬▬๑۩۩๑▬▬▬▬▬▬▬▬▬▬●`]
 let page = 1;
